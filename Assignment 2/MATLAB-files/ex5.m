...
for i=1:length(Q33)
    ...
    SimOut = sim("voima_5.slx",Simulation_Time);
    
    %% figures;
    % u fig
    figure();
    hold on
    plot(SimOut.time, SimOut.u,"LineWidth",1.5, "DisplayName", sprintf("$Q_{44} = %g$", Q44(i)))
    ...
    saveas(gcf, sprintf("Plots\\u_statefeedback_integral_%s.png", int2str(i)))
    ...
    %end
    plot(SimOut.time, SimOut.pkp,"LineWidth",1.5, 'DisplayName', sprintf("$Q_{44} = %g$", Q44(i)))
